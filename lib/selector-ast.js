...
function normalizeNthChildSelector ( selectors ) {
	...
	selectors.walkTags(( number ) => {
		...
		if ( number.type === 'tag' && Number.isInteger(position) && position >= 0 && !previousSelector && !nextSelector ) {
			...
			number.parent.insertBefore(number, parser.combinator({ value: '+' }));
		...
		if ( NTH_CHILD_QUERY_REGEX.test(number.value) ) {
			...
			const [ , negative = '', regexNumber = 1 ] = number.value.match(NTH_CHILD_QUERY_REGEX);
			...
			if ( isInvalidCombinator ) {
				...
				if ( isAlone && negative === '' ) {
					number.parent.insertBefore(number, parser.combinator({ value: '+' }));
					number.parent.insertBefore(number, parser.tag({ value: regexNumber }));
				}
...
export default ( query ) => parser(( selectors ) => {
